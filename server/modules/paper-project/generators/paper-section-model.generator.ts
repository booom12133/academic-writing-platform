...
export class PaperSectionModelGenerator {
  ...
  async generate(input: { context: string; targetWords?: number; instruction?: string }) {
    const prompt = `Draft an academic section as strict JSON {content,integrityWarnings}. Never invent citations, DOI, samples, measurements, coefficients, p-values, confidence intervals, experiments or findings. Use 【待实证结果补充】 or prospective language where results are absent. Target words: ${input.targetWords ?? 'appropriate'}. Context: ${input.context}${input.instruction ? `\nInstruction: ${input.instruction}` : ''}`;
