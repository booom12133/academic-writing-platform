...
export class ResearchPlanGenerator {
  ...
  async generate(profile: ProjectProfileV1, instructions?: string) {
    ...
    const base = `Create a discipline-neutral research plan as strict JSON for this profile: ${JSON.stringify(profile)}. Required ResearchPlanV1 field shape: ${JSON.stringify(shape)}. hypotheses and propositions are optional and may be omitted; the other top-level fields are required. Planned or expected results must not be stated as observed facts.${instructions ? ` Instructions: ${instructions}` : ''}`;
