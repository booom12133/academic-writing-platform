...
export class AcademicIntegrityValidator {
  validateModelOnly(content: string): void {
    ...
    const fabricatedResult = /(?:p\s*[<=>]\s*0?\.\d+|\bn\s*=\s*\d+|(?:sample size|样本量)\s*(?:was|is|of|为|=)\s*\d+|\b\d+\s+(?:participants?|respondents?|subjects?|samples?)\b|\b(?:surveyed|interviewed|recruited|enrolled|tested)\s+\d+\b|\bobserved\s+(?:an?\s+)?\d+(?:\.\d+)?%|(?:regression\s+)?coefficients?\s*(?:was|were|=|of)\s*-?\d|(?:\d+(?:\.\d+)?%\s*)?(?:confidence interval|CI)\s*(?:was|is|=|of)?\s*[\[(]?\s*-?\d+(?:\.\d+)?|(?:results?|findings?)\s+(?:showed|indicated|demonstrated|revealed)|(?:研究|实验)?结果(?:显示|表明)|experiment (?:found|showed)|实验结果表明)/i;
