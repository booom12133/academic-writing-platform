...
export const manuscriptWarningCodeSchema = z.enum([
...
export const exportManifestV1Schema = z.object({
  ...
  warnings: z.array(z.object({ code: manuscriptWarningCodeSchema, sectionId: z.string().uuid().optional(), revisionId: z.string().uuid().optional() }).strict()).max(10_000),
...
export const createPaperExportRequestSchema = z.object({
  format: z.literal('DOCX'),
  mode: z.enum(['DRAFT', 'CLEAN']),
  templateKey: z.literal('generic-academic-v1'),
  expectedManuscriptFingerprint: fingerprint,
  acknowledgedWarningCodes: z.array(manuscriptWarningCodeSchema).max(20).optional(),
}).strict().superRefine((value, context) => {
  if (value.acknowledgedWarningCodes && new Set(value.acknowledgedWarningCodes).size !== value.acknowledgedWarningCodes.length) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: 'Warning acknowledgements must be unique.' });
  }
});
