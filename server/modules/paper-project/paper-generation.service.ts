...
export class PaperGenerationService{
  ...
  async generate(userId:string,projectId:string,sectionId:string,body:unknown):Promise<PaperGenerationResponse>{
    ...
    if(parsed.data.sourceStrategy==='MODEL_ONLY'){
      ...
      const revision=await this.repository.appendRevision(userId,projectId,sectionId,parsed.data.expectedCurrentRevisionNumber,{content:generated.result.content,baseRevisionId:base?.id,origin,sourceStrategy:'MODEL_ONLY',actualSupportMode:'AI_DRAFT',supportState:'NOT_CLAIMED',citations:[],bibliography:[],evidenceTrace:[],generationMetadata:{...generated.metadata,operation:parsed.data.operation,contextFingerprint:createHash('sha256').update(context.text).digest('hex'),includedSectionIds:context.includedSectionIds},warnings:[...context.warnings,...generated.result.integrityWarnings],rewriteInstruction:parsed.data.instructions});
    ...
    try{
      ...
      const revision=await this.repository.appendRevision(userId,projectId,sectionId,parsed.data.expectedCurrentRevisionNumber,{content:generated.content,baseRevisionId:base?.id,origin,sourceStrategy:parsed.data.sourceStrategy,actualSupportMode:classified.actualSupportMode,supportState:'VALID',citations:generated.citations,bibliography:generated.bibliography,evidenceTrace:generated.evidenceTrace,generationMetadata:{...generated.generation,operation:parsed.data.operation,selectedVersionIds:generated.provenance.selectedVersionIds,retrievalProfile:generated.provenance.retrievalProfile},warnings:classified.warnings,rewriteInstruction:parsed.data.instructions});
