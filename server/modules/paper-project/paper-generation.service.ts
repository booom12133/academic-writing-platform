...
export class PaperGenerationService{
  ...
  async generate(userId:string,projectId:string,sectionId:string,body:unknown):Promise<PaperGenerationResponse>{
    ...
    try{
      ...
      const revision=await this.repository.appendRevision(userId,projectId,sectionId,parsed.data.expectedCurrentRevisionNumber,{content:generated.content,baseRevisionId:base?.id,origin,sourceStrategy:parsed.data.sourceStrategy,actualSupportMode:classified.actualSupportMode,supportState:'VALID',citations:generated.citations,bibliography:generated.bibliography,evidenceTrace:generated.evidenceTrace,generationMetadata:{...generated.generation,operation:parsed.data.operation,selectedVersionIds:generated.provenance.selectedVersionIds,retrievalProfile:generated.provenance.retrievalProfile,citationPlacements:generated.citationPlacements,contextWarnings:context.warnings},warnings:[...context.warnings,...classified.warnings],rewriteInstruction:parsed.data.instructions});
