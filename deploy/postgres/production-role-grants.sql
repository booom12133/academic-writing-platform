...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_catalog.pg_roles WHERE rolname = 'academic_writing_backup'
  ) THEN
    CREATE ROLE academic_writing_backup
      LOGIN NOINHERIT NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION NOBYPASSRLS;
  END IF;
END
$roles$;
...
ALTER ROLE academic_writing_app
  LOGIN NOINHERIT NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION NOBYPASSRLS;
ALTER ROLE academic_writing_backup
  LOGIN NOINHERIT NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION NOBYPASSRLS;

REVOKE academic_writing_db_owner FROM academic_writing_migrator, academic_writing_app, academic_writing_backup;
REVOKE academic_writing_migrator FROM academic_writing_app, academic_writing_backup;
REVOKE academic_writing_app FROM academic_writing_migrator, academic_writing_backup;
REVOKE academic_writing_backup FROM academic_writing_db_owner, academic_writing_migrator, academic_writing_app;
...
BEGIN
  ...
  EXECUTE format(
    'GRANT CONNECT ON DATABASE %I TO academic_writing_backup',
    current_database()
  );
END
$database_privileges$;
...
REVOKE ALL ON SCHEMA public, drizzle FROM academic_writing_backup;
...
GRANT USAGE ON SCHEMA public, drizzle TO academic_writing_backup;
...
GRANT SELECT ON ALL TABLES IN SCHEMA public, drizzle TO academic_writing_backup;
GRANT SELECT ON ALL SEQUENCES IN SCHEMA public, drizzle TO academic_writing_backup;
...
ALTER DEFAULT PRIVILEGES FOR ROLE academic_writing_migrator IN SCHEMA public
  GRANT SELECT ON TABLES TO academic_writing_backup;
ALTER DEFAULT PRIVILEGES FOR ROLE academic_writing_migrator IN SCHEMA public
  GRANT SELECT ON SEQUENCES TO academic_writing_backup;
ALTER DEFAULT PRIVILEGES FOR ROLE academic_writing_migrator IN SCHEMA drizzle
  GRANT SELECT ON TABLES TO academic_writing_backup;
ALTER DEFAULT PRIVILEGES FOR ROLE academic_writing_migrator IN SCHEMA drizzle
  GRANT SELECT ON SEQUENCES TO academic_writing_backup;
