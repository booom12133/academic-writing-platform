...
function runRestoreVerify({
  ...
  const libpqEnv = verifiedLibpqEnvironment(env, databaseUrl);
  const restore = spawnSync(invocations[0].command, invocations[0].args, {
    stdio: 'inherit',
    env: libpqEnv,
  });
  ...
  const verify = spawnSync(invocations[1].command, invocations[1].args, {
    encoding: 'utf8',
    env: libpqEnv,
  });
