...
const VERIFY_QUERY = `SELECT CASE WHEN EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'vector') THEN 't' ELSE 'f' END || '|' || (SELECT count(*) FROM information_schema.tables WHERE table_schema = 'public' AND table_name IN ('app_users','tasks','point_records','recharge_orders','knowledge_documents','knowledge_document_versions','knowledge_chunks','knowledge_source_records','knowledge_source_external_links','knowledge_metadata_assertions','knowledge_imports','knowledge_embedding_indexes','knowledge_chunk_embeddings','zotero_connections','paper_projects','paper_outline_nodes','paper_sections','paper_section_revisions','paper_project_sources','paper_exports')) || '|' || (SELECT count(*) FROM drizzle.__drizzle_migrations) || '|' || current_database();`;
