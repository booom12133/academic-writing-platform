...
async function runDefaultLint() {
  ...
  const code = await runCommand(npx.command, [...npx.args, 'concurrently', ...commands]);
  process.exit(code);
}
...
async function runSelectiveLint(inputFiles) {
  ...
  if (eslintFiles.length > 0) {
    const npx = toolInvocation('npx');
    tasks.push(runCommand(npx.command, [...npx.args, 'eslint', '--quiet', ...eslintFiles]));
  }
  ...
  if (stylelintFiles.length > 0 && !canRunStylelint()) {
    ...
    const npx = toolInvocation('npx');
    tasks.push(runCommand(npx.command, [...npx.args, 'stylelint', '--quiet', ...stylelintFiles]));
  }

  if (clientTypeFiles.length > 0) {
    const npm = toolInvocation('npm');
    tasks.push(runCommand(npm.command, [...npm.args, 'run', 'type:check:client']));
  }

  if (serverTypeFiles.length > 0) {
    const npm = toolInvocation('npm');
    tasks.push(runCommand(npm.command, [...npm.args, 'run', 'type:check:server']));
  }
