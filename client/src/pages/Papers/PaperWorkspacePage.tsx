...
import {
  ...
  canMoveOutlineSibling,
  getOutlineControlLabel,
  moveOutlineSibling,
    ...
    <main className="max-w-[1500px] mx-auto px-6 py-6">
      ...
      <section className="my-4 grid gap-4 lg:grid-cols-3">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4">
          ...
          <div className="mt-3 space-y-2">{orderEditableOutline(proposal).map((node) => <div key={node.clientKey} className="flex items-center gap-1"><input aria-label={getOutlineControlLabel('title',node.title)} value={node.title} onChange={(event) => setProposal((current) => current.map((item) => item.clientKey === node.clientKey ? { ...item, title: event.target.value } : item))} className={`min-w-0 flex-1 rounded-md border border-slate-300 px-3 py-2 text-sm ${node.parentClientKey ? 'ml-4' : ''}`} /><button aria-label={getOutlineControlLabel('up',node.title)} disabled={!canMoveOutlineSibling(proposal,node.clientKey,'up')} className="rounded border px-2 py-1 text-xs disabled:opacity-40" onClick={() => setProposal((current) => moveOutlineSibling(current,node.clientKey,'up'))}>↑</button><button aria-label={getOutlineControlLabel('down',node.title)} disabled={!canMoveOutlineSibling(proposal,node.clientKey,'down')} className="rounded border px-2 py-1 text-xs disabled:opacity-40" onClick={() => setProposal((current) => moveOutlineSibling(current,node.clientKey,'down'))}>↓</button><button aria-label={getOutlineControlLabel('remove',node.title)} className="rounded border border-red-200 px-2 py-1 text-xs text-red-600" onClick={() => setProposal((current) => removeOutlineSubtree(current,node.clientKey))}>移除</button></div>)}</div>
