  ...
  return (
    <main className="max-w-[1500px] mx-auto px-6 py-6">
      ...
      <section className="my-4 grid gap-4 lg:grid-cols-3">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4">
          <div className="flex justify-between"><h2 className="font-semibold">论文大纲</h2><button className="text-sm text-blue-600" onClick={() => void run(async () => {
          ...
          <div className="mt-3 space-y-2">{proposal.map((node, index) => <input key={node.clientKey} value={node.title} onChange={(event) => setProposal((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, title: event.target.value } : item))} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" />)}</div>
          {proposal.length > 0 && <button className={`${button} mt-2 bg-slate-800 text-white`} onClick={() => void run(async () => {
            const result = await api.saveOutline(projectId, { expectedLockVersion: project.lockVersion, nodes: proposal });
            setOutline(result.nodes);
            setProject({ ...project, lockVersion: result.lockVersion });
            setProposal([]);
          })}>保存大纲</button>}
        </div>
